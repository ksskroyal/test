 ...
 * 
 * Realisation of a server to work with CookingRecipe JSON data remotely.
 * 
 */
...
/**
 * @brief Function to randomly generate a JSON data with a given number of CookingRecipe
...
void req_execute(const httplib::Request&, httplib::Response&, myjson&,Generator&);
...
int main()
{
 ...
 server.Post("/execute", [&](const httplib::Request& req, httplib::Response& res){
  ...
  server.Post("/shutdown", req_shutdown);
...
void req_generate(const httplib::Request& req, httplib::Response& res, myjson& data, Generator& gen){
  ...
  std::cout << "generating " << count << " CookingRecipe" << std::endl;
...
void req_execute(const  httplib::Request& req, httplib::Response&, myjson& data, Generator& gen)
{
    std::string count1 = req.get_param_value("t1");
    int  count_1= std::stoi(count1);
    std::cout << typeid(count_1).name() << count_1;
    std::string count2 = req.get_param_value("t2");
    int  count_2 = std::stoi(count2);
    std::string count3 = req.get_param_value("c1");
    int  count_3= std::stoi(count3);
    std::string count4 = req.get_param_value("c2");
    int  count_4= std::stoi(count4);
  
    data.Execute(count_1,count_2,count_3,count_4);
    
}
void req_add(const httplib::Request& req, httplib::Response&, myjson& data, Generator& gen){
  std::string restName = req.body;
    CookingRecipe toAdd = gen.genCookingRecipe();
  toAdd.name = restName;
  toAdd.id = CookingRecipe::getID(restName, toAdd.ingredient.size());
  data.Add(toAdd);
}
...
void req_find(const httplib::Request& req, httplib::Response& res, myjson& data){
  ...
  json resJson = data.FindCookingRecipe(id);
  if(resJson == json({})){
    res.status = 404;
    return;
  }
